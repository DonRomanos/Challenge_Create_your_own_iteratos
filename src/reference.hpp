...
namespace reference
{
	// The heart of my implementation, this is where all the logic happens :)
	...
	struct iterator_base
	{
		...
		// We can use the default comparison operator which will also synthesize equality comparison for us.
	...
	struct MyContainer
	{
		// This is the beauty of this implementation we only need two typedefs here :) (The bool can still be replaced by some more meaningful name...)
...
// A custom 3 way comparison is not used to synthesize equality comparisons, reasons are performance, e.g. lexicographical comparison of strings compares each character, while testing for equality you can test for equal sizes first then no loop is required.
...
// Enabling the signature of a function based on a template parameter is possible using SFINAE e.g. const and non-const functions
